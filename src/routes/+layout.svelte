...
<div class="flex flex-col min-h-screen box-border">
    <main class="flex-1 p-2 sm:p-4 flex flex-col gap-4 max-w-full">
    ...
    {#if showPopup}
        ...
        >
            <div
                class="popup backdrop-blur-md rounded-lg w-full sm:max-w-lg border border-glass animate-fade-in pointer-events-auto transition-all duration-200 flex flex-col box-border"
                class:collapsed
                style="position: absolute; left: {currentX}px; top: {currentY}px;"
            >
                <div
                    class="drag-handle relative flex justify-between items-center p-4 sm:p-6 cursor-grab group"
                    role="button"
                    tabindex="0"
                ...
                {#if !collapsed}
                    <div class="p-4 sm:p-6 pt-0 flex-1 max-w-full box-border overflow-y-auto">
                        <Controls />
                    </div>
                {/if}
            </div>
